...
const handleSignoff = async (status) => {
  ...
  try {
    ...
    console.error(err)
    toastStore.showToast('簽核失敗: ' + (err.response?.data?.error || err.message), 'error')
  }
}
